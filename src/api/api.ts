import { IFilter, IPoke, IPokeAPI, IPokeSpeciesAPI, IReducedPokeAPI } from './../interfaces/PokeInterface';
const BASE_URL = "https://pokeapi.co/api/v2/pokemon"; 
const TYPE_URL = "https://pokeapi.co/api/v2/type/";

export async function getPokeList(offset: number = 0, limit: number = 10) : Promise<IPoke[]>
{
    const response = await fetch(BASE_URL + "?limit="+limit+"&offset="+offset);
    const data: {count: number, results: IReducedPokeAPI[]} = await response.json();
    return await Promise.all(
        data.results.map(async (result: IReducedPokeAPI) => getPoke(result)))
}


export async function getAllPokeWNameAndEleFilter(name:string, eleNames: IFilter[]) : Promise<IPoke[]>
{
    ...
    if(eleNames.length > 0)
    {
        for await (const ele of eleNames)
        {
            ...
            const data: {pokemon: {pokemon: IReducedPokeAPI}[]} = await response.json()  
            const tempP = data.pokemon.filter(p => p.pokemon.name.indexOf(name)>=0)
                .slice(0, 20) 
                .map(async (result:{pokemon: IReducedPokeAPI}) => getPoke(result.pokemon))
...
async function getAllPokeWithName(name: string) : Promise<IPoke[]>
{
    ...
    const data: {count: number, results: IReducedPokeAPI[]} = await response.json()
    const filtersPokes = data.results.filter((p:IReducedPokeAPI) => p.name.includes(name))
    return await Promise.all(
        filtersPokes.slice(0, 20).map(async (result: IReducedPokeAPI) => getPoke(result)))
}
...
async function getPoke(result: IReducedPokeAPI) : Promise<IPoke>
{
    ...
    const pokeSpeciesData: IPokeSpeciesAPI = await fetch(pokeData.species.url)
        .then(data => data.json())
    return {
        url: pokeData.url,
        id: pokeData.id,
